...
class VLATrainer(TrainerUtils):
    ...
    def _train_step(self, batch_vla, batch_vlm=None):
        ...
        with self.accelerator.accumulate(self.model):
            ...
            # Capture LatentSteering gradient signal via output tensor (DeepSpeed-proof)
            if self.accelerator.sync_gradients:
                raw_model = self.accelerator.unwrap_model(self.model)
                if hasattr(raw_model, 'latent_steering') and raw_model.latent_steering is not None:
                    steer = raw_model.latent_steering
                    fused_out = getattr(steer, '_last_fused_out', None)
                    grad_ok = 0.0
                    grad_norm_val = 0.0
                    if fused_out is not None and fused_out.grad is not None:
                        grad_ok = 1.0
                        grad_norm_val = fused_out.grad.detach().norm(2).item()
                    _steering_grads = {
                        "steering/grad_signal": grad_ok,
                        "steering/grad_fused_norm": grad_norm_val,
                    }
        ...
        # Include LatentSteering diagnostics if present
        phase3_diag = getattr(self.model, '_phase3_diag', {})
        if phase3_diag:
            for k, v in phase3_diag.items():
                if isinstance(v, (int, float)):
                    metrics[f"steering/{k}"] = v
        metrics.update(_steering_grads)
        return metrics
