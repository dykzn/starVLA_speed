...
def build_param_lr_groups(model, cfg):
    ...
    # Phase 3.1: cross-attention specific LR (asymmetric LR for attn1 Q/K/V matrices)
    cross_attn_lr = lr_cfg.get("cross_attn", 0.0)
    if cross_attn_lr > 0.0 and hasattr(model, 'action_model'):
        ca_params = []
        am = model.action_model
        if hasattr(am, 'model') and hasattr(am.model, 'transformer_blocks'):
            for block in am.model.transformer_blocks:
                for param_name, param in block.attn1.named_parameters():
                    if any(x in param_name for x in ['to_q', 'to_k', 'to_v']):
                        if id(param) not in frozen_params:
                            ca_params.append(param)
        if ca_params:
            param_groups.append({"params": ca_params, "lr": cross_attn_lr, "name": "cross_attn"})
            used_params.update(id(p) for p in ca_params)
            print(f"🔥 Cross-attention asymmetric LR: {cross_attn_lr:.1e} ({len(ca_params)} params)")

    for module_name, lr in lr_cfg.items():
        if module_name in ("base", "cross_attn"):
            continue
        ...
        try:
            for attr in module_name.split("."):
                module = getattr(module, attr)
            # filter out frozen AND already-used parameters
...
class TrainerUtils:
    ...
    @staticmethod
    def setup_wm_warmup(model):
        """Phase 1 WM warm-up: freeze all, then unfreeze only wm_projector + cross-attn.

        After calling freeze_backbones(), this selectively re-enables training for:
          - wm_projector (LayerNorm + Linear 3072→2560)
          - Action Head cross-attention (attn1 in each DiT transformer block)

        Everything else (VLM, WM, FFN, norms, project_layers) stays frozen.
        """
        # Step 1: freeze every parameter in the model
        for p in model.parameters():
            p.requires_grad = False

        # Step 2: unfreeze wm_projector
        if hasattr(model, 'wm_projector') and model.wm_projector is not None:
            for p in model.wm_projector.parameters():
                p.requires_grad = True
            print("🔥 WM warm-up: wm_projector UNFROZEN")

        # Step 3: unfreeze Action Head cross-attention layers (attn1)
        if hasattr(model, 'action_model') and hasattr(model.action_model, 'model'):
            unfrozen_count = 0
            for block in model.action_model.model.transformer_blocks:
                if hasattr(block, 'attn1'):
                    for p in block.attn1.parameters():
                        p.requires_grad = True
                    unfrozen_count += 1
            print(f"🔥 WM warm-up: cross-attention UNFROZEN in {unfrozen_count} DiT blocks")

        # Step 4: keep action_encoder, action_decoder frozen for stability
        return model

    @staticmethod
    def setup_action_head_finetune(model):
        """Phase 2 Action Head Fine-tuning: freeze all, then unfreeze entire action head + wm_projector.

        After calling freeze_backbones(), this selectively re-enables training for:
          - ALL action_model parameters (DiT blocks, action_encoder, action_decoder,
            state_encoder, future_tokens, position_embedding, norms, FFN)
          - wm_projector (LayerNorm + Linear 3072→2560)
          - cfg_mask_token (learnable mask for CFG)

        Everything else (VLM, WM, project_layers) stays frozen.

        Introduces CFG (Classifier-Free Guidance): during batch construction,
        VLM tokens are randomly replaced with a learnable mask token (controlled
        by cfg_dropout_prob), forcing the DiT to rely on WM physical features
        when high-level semantics are unavailable.
        """
        # Step 1: freeze every parameter in the model
        for p in model.parameters():
            p.requires_grad = False

        # Step 2: unfreeze entire action_model (all DiT blocks + encoders + decoders)
        if hasattr(model, 'action_model'):
            unfrozen_action = 0
            for p in model.action_model.parameters():
                p.requires_grad = True
                unfrozen_action += p.numel()
            print(f"🔥 Phase2 Fine-tuning: entire action_model UNFROZEN ({unfrozen_action / 10**6:.1f}M params)")

        # Step 3: unfreeze wm_projector + wm_scale (if exists)
        if hasattr(model, 'wm_projector') and model.wm_projector is not None:
            for p in model.wm_projector.parameters():
                p.requires_grad = True
            print("🔥 Phase2 Fine-tuning: wm_projector UNFROZEN")
        # Step 4: unfreeze cfg_mask_token (learnable mask for CFG, if it exists)
        if hasattr(model, 'cfg_mask_token') and model.cfg_mask_token is not None:
            model.cfg_mask_token.requires_grad = True
            print("🔥 Phase2 Fine-tuning: cfg_mask_token UNFROZEN")

        return model

    @staticmethod
    def setup_cross_attn_finetune(model):
        """Phase 3 Cross-Attention Fine-tuning: same as Phase 2 but also unfreezes project_layers.

        Phase 3 uses attention-mask based CFG (not feature zeroing) to enable
        dynamic attention allocation between VLA and WM features. The project_layers
        that map VLM hidden states to DiT latent space are unfrozen to allow the
        cross-attention to adapt its feature mixing.
        """
        # First apply Phase 2 unfreezing (action_model + wm_projector + cfg_mask_token)
        model = TrainerUtils.setup_action_head_finetune(model)

        # Phase 3: also unfreeze project_layers + vla_norm_layers for cross-attention adaptation
        if hasattr(model, 'project_layers') and model.project_layers is not None:
            unfrozen_proj = 0
            for p in model.project_layers.parameters():
                p.requires_grad = True
                unfrozen_proj += p.numel()
            print(f"🔥 Phase3 Cross-Attn: project_layers UNFROZEN ({unfrozen_proj / 10**6:.1f}M params)")
        if hasattr(model, 'vla_norm_layers') and model.vla_norm_layers is not None:
            unfrozen_norm = 0
            for p in model.vla_norm_layers.parameters():
                p.requires_grad = True
                unfrozen_norm += p.numel()
            print(f"🔥 Phase3 Cross-Attn: vla_norm_layers UNFROZEN ({unfrozen_norm / 10**6:.1f}M params)")
    ...
    @staticmethod
    def init_ema(model, decay=0.999):
        """Initialize EMA shadow parameters for all trainable parameters."""
        from copy import deepcopy
        ema_params = {}
        for name, p in model.named_parameters():
            if p.requires_grad:
                ema_params[name] = deepcopy(p.data).cpu()
        print(f"  EMA initialized: {len(ema_params)} parameter groups, decay={decay}")
        return ema_params
            ...
            for name, p in model.named_parameters():
                if p.requires_grad and name in ema_params:
                    ema_params[name].copy_(
                        decay * ema_params[name] + (1.0 - decay) * p.data.cpu()
                    )

    @staticmethod
    def apply_ema_weights(model, ema_params):
        """Temporarily replace model weights with EMA weights (for evaluation)."""
        for name, p in model.named_parameters():
            if p.requires_grad and name in ema_params:
                p.data.copy_(ema_params[name].to(p.device, dtype=p.dtype))

    @staticmethod
    def save_ema_checkpoint(model, ema_params, save_path):
        """Save EMA weights as a checkpoint file."""
        state_dict = {}
        for name, p in model.named_parameters():
            if p.requires_grad and name in ema_params:
                state_dict[name] = ema_params[name]
            else:
                state_dict[name] = p.data.cpu()
        # Save non-parameter state (buffers) from model
    ...
    def load_pretrained_backbones(model, checkpoint_path=None, reload_modules=None):
        ...
        try:
            if _is_safetensors_path(checkpoint_path):
                ...
                _start = _time.time()
                if not dist.is_initialized() or dist.get_rank() == 0:
                    _fsize = os.path.getsize(checkpoint_path) / (1024**3)
                    print(f"⏳ reading checkpoint file ({_fsize:.1f} GB), this may take several minutes...")
                checkpoint = torch.load(checkpoint_path, map_location="cpu")
                _elapsed = _time.time() - _start
                if not dist.is_initialized() or dist.get_rank() == 0:
                    print(f"✅ checkpoint loaded in {_elapsed:.1f}s")
