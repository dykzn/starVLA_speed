...
class _Wan2_Interface(nn.Module):
    ...
    def __init__(self, config: Optional[dict] = None, **kwargs):
        ...
        # --- Text encoder: UMT5-XXL (optional — FastWAM feature extraction can work without it) ---
        self.tokenizer = None
        ...
        try:
            ...
            logger.info("  Text encoder (UMT5-XXL) loaded successfully")
        except Exception as e:
            logger.warning(f"  Text encoder not available ({e}), will use zero embeddings."
                           f"  WM features will not be language-conditioned.")

        # --- DiT transformer ---
        fastwam_transformer = wm_cfg.get("fastwam_transformer_path", None)
        if fastwam_transformer:
            logger.info(f"Loading Fast-WAM fine-tuned transformer from {fastwam_transformer}")
            self.transformer = WanTransformer3DModel.from_pretrained(
                fastwam_transformer, torch_dtype=torch.bfloat16, local_files_only=True
            )
        else:
            self.transformer = WanTransformer3DModel.from_pretrained(
    ...
    def _encode_text(self, instructions, max_length=512):
        """Encode text instructions using UMT5. Falls back to zero embeddings."""
        if self.text_encoder is None:
            # Zero embedding placeholder — WM features will not be language-conditioned
            B = len(instructions) if isinstance(instructions, (list, tuple)) else 1
            device = next(self.transformer.parameters()).device
            return torch.zeros(B, max_length, 4096, device=device, dtype=torch.bfloat16)
    ...
    def _encode_images_vae(self, images, num_frames=None):
        ...
        # Use compact resolution to keep DiT seq_len ≤ rope_max_seq_len (1024)
        # VAE spatial scale=8, DiT patch=[1,2,2] → grid = 256/8/2 = 16 per side
    ...
    def build_inputs(self, images, instructions, **kwargs):
        ...
        # Wan2.2 3D rope needs T_latent >= 2; temporal downsample=4, so need >=5 frames
        # (N-1)//4+1 >= 2  ⇒  N >= 5
        min_frames = int(self.vae_scale_factor_temporal) + 1  # 5
        latents = self._encode_images_vae(images, num_frames=min_frames)
