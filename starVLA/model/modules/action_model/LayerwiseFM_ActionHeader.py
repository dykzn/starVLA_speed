...
class LayerwiseFlowmatchingActionHead(nn.Module):
    ...
    def forward(
        ...
        wam_latents: list = None,
        vla_mask: torch.Tensor = None,
    ):
        """
        ...
        wam_latents: optional list of Tensor from Fast-WAM world model
        vla_mask: Phase 3 attention mask for VLA tokens (B, vl_seq_len), 1=keep 0=mask
        """
        ...
        # Build encoder features: VLM + optional world model
        if wam_latents is not None:
            encoder_features = [
                torch.cat([vl_embs_list[i], wam_latents[i]], dim=1)
                for i in range(len(vl_embs_list))
            ]
            # Extend attention mask to cover WM tokens (all real, no padding)
            if encoder_attention_mask is not None:
                wm_mask = torch.ones(
                    encoder_attention_mask.shape[0], wam_latents[0].shape[1],
                    dtype=encoder_attention_mask.dtype, device=encoder_attention_mask.device
                )
                encoder_attention_mask = torch.cat([encoder_attention_mask, wm_mask], dim=1)
            # Phase 3: apply VLA attention mask on top of padding mask
            # (1=keep, 0=mask → -inf in softmax; preserves original padding mask)
            if vla_mask is not None and encoder_attention_mask is not None:
                vl_seq_len = vla_mask.shape[1]
                orig_dtype = encoder_attention_mask.dtype
                orig_vla = encoder_attention_mask[:, :vl_seq_len].float()
                encoder_attention_mask[:, :vl_seq_len] = (
                    orig_vla * vla_mask.to(dtype=torch.float, device=orig_vla.device)
                ).to(dtype=orig_dtype)
        else:
            encoder_features = vl_embs_list
        ...
        for layer_idx, layer in enumerate(self.model.transformer_blocks):
            model_output = layer(
                ...
                encoder_hidden_states=encoder_features[layer_idx],
    ...
    def predict_action(
        ...
        # Build encoder features once (same for all denoising steps)
        if wam_latents is not None:
            encoder_features = [
                torch.cat([vl_embs_list[i], wam_latents[i]], dim=1)
                for i in range(len(vl_embs_list))
            ...
            if encoder_attention_mask is not None:
                ...
                encoder_attention_mask = torch.cat([encoder_attention_mask, wm_mask], dim=1)
            # Phase 3: during inference, no VLA masking (full dual-stream)
        else:
            encoder_features = vl_embs_list
