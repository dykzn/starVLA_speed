...
# Ensure LIBERO-plus is found before any other libero installation
...
class Args:
    ...
    # Resume support: only evaluate tasks in [start_idx, end_idx)
    start_idx: int = 0
    end_idx: int = -1   # -1 means all tasks
    ...
    save_video: bool = False  # Set to True to enable video saving
...
def eval_libero(args: Args) -> None:
    ...
    # Respect start_idx / end_idx for resume
    start = max(0, args.start_idx)
    end = num_tasks_in_suite if args.end_idx < 0 else min(args.end_idx, num_tasks_in_suite)
    logging.info(f"Task range: [{start}, {end}) / {num_tasks_in_suite}")
    ...
    if total_episodes > 0:
        ...
        logging.warning(f"WARNING: No episodes completed! start={start}, end={end}, n_tasks={num_tasks_in_suite}, num_trials={args.num_trials_per_task}")
