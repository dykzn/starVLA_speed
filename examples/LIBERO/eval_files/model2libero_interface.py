...
class ModelClient:
    ...
    def step(self, example: Optional[dict] = None, step: int = 0, **kwargs) -> dict:
        """One env step.

        Args:
            example: dict with keys ``image`` (list of np.uint8 HWC arrays) and ``lang`` (str).
                It may be omitted while consuming the cached action chunk.
        ...
        if example is not None and example.get("lang", None) != self.task_description:
            # Preserve the original adapter behavior for callers that switch
            # tasks without explicitly calling reset().
            self.reset(example.get("lang", None))
            refresh_chunk = True

        # Once a chunk has been fetched, the next action is already local.  Do
        # not resize images, serialize a websocket request, or even require an
        # image on the remaining steps of this chunk.
        if not refresh_chunk:
        ...
        if example is None:
            raise ValueError("An example is required when refreshing the action chunk")
