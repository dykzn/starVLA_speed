...
class PolicyServerWrapper:
    ...
    def __init__(
        ...
        # This process is inference-only.  Disabling parameter gradients here
        # avoids accidental autograd bookkeeping for framework variants whose
        # predict_action implementation is not decorated with inference_mode.
        framework.requires_grad_(False)

        # Optional: inject Fast-WAM world model after loading
        if enable_world_model:
            logging.info("PolicyServerWrapper: loading Fast-WAM world model...")
            ...
            wm_cfg = OmegaConf.create({
                "framework": {
                    "world_model": {
                        ...
                        "fastwam_transformer_path": "/data3/dengyongkang/my_project/FastWAM/checkpoints/Wan2.2-FastWAM-LIBERO-diffusers/transformer",
                        "extract_layers": [-1],  # single last layer for fast inference; set to list(range(30)) for training
                    }
                }
            })
            framework.config.framework.world_model = wm_cfg.framework.world_model
            framework.world_model = get_world_model(config=framework.config)
            for p in framework.world_model.parameters():
                p.requires_grad = False
            framework.world_model.to(device).eval()
            logging.info("PolicyServerWrapper: world model loaded.")
    ...
    def predict_action(
        ...
        **kwargs,
        ...
        # Keep the guarantee at the deployment boundary as well as inside
